    ...
    def execute(self):
        ...
        for usuario in usuarios:
            ...
            for inmueble in inmuebles:
                if inmueble.coste_tpd_actualizado:
                    # El coste TPD actual es el correcto. Ninguna acción que hacer.
                    print('El coste TPD actual es el correcto. Ninguna acción que hacer.')

                    # # voy a mostrarlo, a ver cómo es: # puedes borrar esto al acabar
                    # costes_tpd = models.CosteInmuebleTPD.objects.all()
                    # for coste_tpd in costes_tpd:
                    #     print('Coste id: {}'.format(coste_tpd.id))
                    #     print('Coste, inmueble origen id: {}'.format(coste_tpd.inmueble_asociado_id))
                    #     print('Coste, inmueble origen nombre: {}'.format(coste_tpd.inmueble_asociado.nombre))

                else:
                    # El coste TPD está desactualizado. Procedo a crear uno.
                    print('El coste TPD está desactualizado.')
                    ...
                    if (primera_fecha_inmueble > primera_fecha_historico) and (
                        ...
                        df_mr.index.freq = 'h'
                        if models.CosteInmuebleTPD.objects.exists():
                            # Existe un Coste TPD antiguo. Procedo a actualizarlo.
                            print('Existe un Coste TPD antiguo. Procedo a actualizarlo.')
                            costes_tpd = models.CosteInmuebleTPD.objects.all()
                            for coste_tpd in costes_tpd:
                                info_coste = calcular_coste_mr_inmueble(df_inm, df_mr, 'TPD')
                                ...
                                ruta_pred = ruta_fich + 'coste_C' + '_TPD_' + id_random_generator() + '.csv'
                                info_coste.get('datos').to_csv(ruta_pred)
                                print('Fichero con ambos dataframes: {}'.format(ruta_pred))  # eliminar esta línea

                                # actualización
                                coste_tpd.coste = info_coste.get('valor')
                                coste_tpd.ruta_costes = ruta_pred
                                coste_tpd.save()
                                # Aviso al inmueble de que ahora ya sí tiene
                                inmueble.coste_tpd_actualizado = True
                                inmueble.save()
                                # print('Actualizado el coste de este inmueble: {}'.format(
                                #     inmueble.coste_tpd_actualizado))  # elimina esta línea

                        else:
                            # No hay Costes TPD antiguos. Procedo a crear uno.
                            print('No hay Costes TPD antiguos. Procedo a crear uno.')
                            ...
                            nuevo_coste_tpd = models.CosteInmuebleTPD.objects.create(inmueble_asociado=inmueble,
                        ...
                        df_mr_crawler = precios_pvpc(primera_fecha_inmueble.isoformat(), ultima_fecha_inmueble.isoformat())
                        ...
                        if models.CosteInmuebleTPD.objects.exists():
                            ...
                            costes_tpd = models.CosteInmuebleTPD.objects.all()
                            for coste_tpd in costes_tpd:
                                info_coste = calcular_coste_mr_inmueble(df_inm, df_mr_crawler, 'TPD')
